...
Section WithG.
  ...
  Inductive eval_exp {VT: @VTyp T V} {DVT: @DefaultVTyp T V VT} : H -> FrameId -> exp -> V -> H -> Prop :=
  | eval_cnum_ :
      forall
        frm z h s t,
  ...
  | eval_new_ :
      forall
        frm r ris rs es h0 h1 rf s t s0 ds p s' d'
  ...
  with fill_slots_par {VT: @VTyp T V} {DVT: @DefaultVTyp T V VT} : H -> FrameId -> FrameId -> list D -> list exp -> AuxV -> H -> Prop :=
  | fill_par_nil :
      forall
        h frm fc,
  ...
  with fill_slots_seq {VT: @VTyp T V} {DVT: @DefaultVTyp T V VT} : H -> FrameId -> list (ScopeId * D) -> list exp -> AuxV -> H -> Prop :=
...
lets into nested parallel lets. *)
  ...
  with fill_slots_rec {VT: @VTyp T V} {DVT: @DefaultVTyp T V VT} : H -> FrameId -> list D -> list exp -> AuxV -> H -> Prop :=
